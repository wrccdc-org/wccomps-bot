...
class TestFindDuplicateFinding:
    """Tests for find_duplicate_finding function."""

    def test_no_duplicate_when_no_findings_exist(self, teams, attack_type):
        """Returns None when no findings exist."""
        existing, uncovered = find_duplicate_finding(
            attack_type=attack_type,
            boxes=["Web Server"],
            teams=teams[:2],
        )
        assert existing is None
        assert uncovered == set(teams[:2])

    def test_no_duplicate_different_attack_type(self, teams, users, attack_type):
        """No duplicate if attack type differs."""
        other_attack, _ = AttackType.objects.get_or_create(name="XSS", defaults={"is_active": True})
    ...
    def test_no_duplicate_no_box_overlap(self, teams, users, attack_type):
        """No duplicate if boxes don't overlap."""
        finding = RedTeamFinding.objects.create(
            attack_type=attack_type,
            affected_boxes=["Web Server"],
            source_ip="10.0.0.1",
    ...
    def test_no_duplicate_no_team_overlap(self, teams, users, attack_type):
        """No duplicate if teams don't overlap."""
        finding = RedTeamFinding.objects.create(
            attack_type=attack_type,
            affected_boxes=["Web Server"],
            source_ip="10.0.0.1",
    ...
    def test_finds_duplicate_with_overlap(self, teams, users, attack_type):
        """Finds duplicate when attack type, box, and team overlap."""
        finding = RedTeamFinding.objects.create(
            attack_type=attack_type,
            affected_boxes=["Web Server"],
            source_ip="10.0.0.1",
        ...
        finding.affected_teams.add(teams[0], teams[1])
        ...
        assert existing == finding
        assert uncovered == {teams[2]}

    def test_returns_uncovered_teams(self, teams, users, attack_type):
        """Returns teams not covered by existing finding."""
        finding = RedTeamFinding.objects.create(
            attack_type=attack_type,
            affected_boxes=["Web Server"],
            source_ip="10.0.0.1",
        ...
        assert existing == finding
        assert uncovered == {teams[1], teams[2]}


class TestMergeSourceIps:
    """Tests for merge_source_ips function."""

    def test_adds_new_single_ip(self, teams, users, attack_type):
        """Adds a new IP to a finding with existing IP."""
        finding = RedTeamFinding.objects.create(
            attack_type=attack_type,
            affected_boxes=["Web Server"],
            source_ip="10.0.0.1",
        ...
        result = merge_source_ips(finding, "10.0.0.2", None)
        assert result is True
        finding.refresh_from_db()
        assert finding.source_ip_pool is not None
        assert set(finding.source_ip_pool.get_ip_list()) == {"10.0.0.1", "10.0.0.2"}

    def test_no_merge_if_ip_already_exists(self, teams, users, attack_type):
        ...
        result = merge_source_ips(finding, "10.0.0.1", None)
